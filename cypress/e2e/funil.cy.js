describe("Validação de requests do Google Tag Manager e Google Analytics", () => {
  let utms, formfields, domain, vlsId, pitch, checkout;

  before(() => {
    cy.fixture("cypress.env.funil.json").then((env) => {
      Cypress.env(env);
      utms =
  ...
  it("Verifica a passagem de UTMs P1 > P2 > PV > Checkout e Fields Forms P2 > PV > Checkout", () => {
    ...
    cy.get("@impulseId").then((impulseId) => {
      cy.url().should("include", domain + "p2" + utms + impulseId);
    });
    ...
    cy.get("@impulseId").then((impulseId) => {
      cy.url().should("include", domain + "pv" + utms + impulseId + formfields);
    });
    cy.wait(2000);
